...
select *, 
case
    when UPPER(Product_name) LIKE '%LOAN%' then 'Loan_type'
    else 'Other_financial_type'
end as Product_type from PK_CONSUMER_COMPLIANTS;


select distinct date_received from PK_CONSUMER_COMPLIANTS;

select distinct SUBSTRING(date_received,7,12) from PK_CONSUMER_COMPLIANTS;

select distinct extract(year from to_date(date_received,'dd-mm-yyyy')) from PK_CONSUMER_COMPLIANTS;

-- 1st jan 2013 to 31st dec 2013 - ppr12
-- 1st jan 2014 to 31st dec 2014 -pr12
-- 1st jan 2015 to 31st dec 2015 -tr12

select *,
    case
        when to_date(date_received,'dd-mm-yyyy') >= to_date('01-01-2013','dd-mm-yyyy') and to_date(date_received,'dd-mm-yyyy') <= to_date('31-12-2013','dd-mm-yyyy') then 'PPR12'
        when to_date(date_received,'dd-mm-yyyy') >= to_date('01-01-2014','dd-mm-yyyy') and to_date(date_received,'dd-mm-yyyy') <= to_date('31-12-2014','dd-mm-yyyy') then 'PR12'
        when to_date(date_received,'dd-mm-yyyy') >= to_date('01-01-2015','dd-mm-yyyy') and to_date(date_received,'dd-mm-yyyy') <= to_date('31-12-2015','dd-mm-yyyy') then 'PR12'
        else 'Invalid range'
    end as period
from PK_CONSUMER_COMPLIANTS;
...
CREATE OR REPLACE TABLE PK_COMSUMER_COMPLIANTS_FOR_AWS LIKE PK_CONSUMER_COMPLIANTS; -- ONLY STRUCTURE GETS COPIED INTO NEW TABLE
SHOW COLUMNS IN PK_COMSUMER_COMPLIANTS_FOR_AWS;

SELECT * FROM PK_COMSUMER_COMPLIANTS_FOR_AWS;

-- LOAD THE DATA FROM STAGING IN DATABASES SECTION
-- LIST STAGE CONFIRMS IF IT IS EMPTY
LIST @AWS_CONSUMER_COMPLAINTS;

-- COPYING INTO STORAGE SERVICE
COPY INTO PK_COMSUMER_COMPLIANTS_FOR_AWS FROM @AWS_CONSUMER_COMPLAINTS
