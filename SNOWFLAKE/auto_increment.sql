...
alter table pk_IDENTITY_COLUMN_EXAMPLE rename to repl_colors;

SELECT * FROM repl_colors;
...
-- when we are inserting new data into table the identity column was never triggered
...
insert into identity_column_example(c_custkey, c_name, c_address, c_nationkey, c_phone, c_acctbal, c_mktsegment, c_comment)
values (219875, 'Customer#000219875', 'sadhesh', 9, '19-505-461-2873', 999.99, 'asjdasgdj', 'sdadtyhkjer');

select * from identity_column_example where c_custkey=219875;
select * from identity_column_example order by id;
--where c_custkey = 219874;

--SYNTAX : alter table <table-name> modify column <column-name> default <new-sequence-name>.nextval;
alter table identity_column_example 
MODIFY COLUMN id  DEFAULT seq1.NEXTVAL;

delete from identity_column_example;
...
UPDATE identity_column_example 
SET id default seq1.nextval;
