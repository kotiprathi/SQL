...
create table if not exists bank_details1_pk(
age int,
job varchar(30),
marital varchar(30),
...
SELECT * FROM bank_details1_pk

insert into bank_details1_pk values(58,"management","married","tertiary","no",213,"yes","no","unknown",5,"may",261,1,-1,0,"unknown","no")

insert into bank_details1_pk values
(44,"technician","single","secondary","no",29,"yes","no","unknown",5,"may",151,1,-1,0,"unknown","no"),
(33,"entrepreneur","married","secondary","no",2,"yes","yes","unknown",5,"may",76,1,-1,0,"unknown","no"),
(47,"blue-collar","married","unknown","no",1506,"yes","no","unknown",5,"may",92,1,-1,0,"unknown","no"),
(33,"unknown","single","unknown","no",1,"no","no","unknown",5,"may",198,1,-1,0,"unknown","no"),bank_details1_pk
...
select * from bank_details1_pk
select count(*) from bank_details1_pk
select age,loan,job from bank_details1_pk
select `default` from bank_details1_pk

select * from bank_details1_pk limit 5
select * from bank_details1_pk where age=33
select * from bank_details1_pk where age=60
select * from bank_details1_pk where age=60 and job='retired'
select * from bank_details1_pk where education='unknown' or marital ='single'
select * from bank_details1_pk where (education='unknown' or marital ='single') and balance<500
select distinct job from bank_details1_pk
select * from bank_details1_pk
select * from bank_details1_pk order by age 
select * from bank_details1_pk order by age desc
select * from bank_details1_pk order by job

#Try to find out sum of balance 
select sum(balance) from bank_details1_pk

#Try to find out avg of balance 
select avg(balance) from bank_details1_pk

#try to find out who is having a min balance 
select * from bank_details1_pk order by balance limit 1
select * from bank_details1_pk where balance in (select min(balance) from bank_details1_pk)
select * from bank_details1_pk where balance = (select min(balance)  from bank_details1_pk)

#try to find out who is having a mamimum balance 
select * from bank_details1_pk order by balance desc limit 1
select * from bank_details1_pk where balance = (select max(balance)  from bank_details1_pk)
select max(balance) from bank_details1_pk 

#try to prepare a list of all the person who is having loan 
select * from bank_details1_pk where loan='yes'

#try to find out average balance for all the people whose job role is admin 
select avg(balance) from bank_details1_pk where job='admin.'

#try to find out a record  without job whose age is below 45 
select * from  bank_details1_pk where job='unknown' and age<45

#try to find out a record where education is primary and person is jobless
select * from bank_details1_pk where education='primary' and job='unknown'

#try to find of a record whose bank account is having a negative balance 
select * from bank_details1_pk where balance<0

#try to find out a record who is not having house at all along with there balance
select balance,housing from bank_details1_pk where housing='no'
...
DELIMITER &&
create procedure select_distinct_job()
BEGIN
	select distinct job from bank_details1_pk;
END &&

call select_all();
call select_distinct_job();

DELIMITER &&
create procedure avg_bal_jobrole1(IN var varchar(20))
BEGIN
	select avg(balance) from bank_details1_pk where job=var;
END &&
...
DELIMITER &&
create procedure sel_edu_job_param(IN var1 varchar(20), In var2 varchar(20))
BEGIN
	select * from bank_details1_pk where education=var1 and job=var2;
END &&

call sel_edu_job_param('primary','admin.');
call sel_edu_job_param('tertiary','retired');

-- VIEWS

create view bank_view as select age,job,marital,education,balance from bank_details1_pk;
